...
public class PostTesting {
	...
	@Before
	public void setUp() {
		post = new Post(76, "feeling thursday!", "http://github.com", LocalDateTime.now(), new Likes(500, null),
				new Comments(null, "wow! looking thin", LocalDateTime.now(), 500));
		post2 = new Post(8, "feeling monday!", "http://github.com", LocalDateTime.now(), new Likes(500, null),
				new Comments(null, "wow! looking thin", LocalDateTime.now(), 500));
		post3 = new Post(9, "feeling monday!", "http://github.com", LocalDateTime.now(), new Likes(500, null),
				new Comments(null, "wow! looking thin", LocalDateTime.now(), 500));
	...
	@Test
	public void badUrl() {
		ResponseEntity<Post> badurl = restTemplate.getForEntity("/post",Post.class);
		assertEquals(HttpStatus.NOT_FOUND , badurl.getStatusCode());
	...
	public void testForUpdatePost() {
		restTemplate.put("/posts/102", post);
		ResponseEntity<List> post = restTemplate.getForEntity("/posts/76", List.class);
		assertThat(post.getStatusCode()).isEqualTo(HttpStatus.OK);
		// assertThat(post.get).isEqualTo(HttpStatus.OK);
	}
	@Test
	public void testForUpdatePostWithWrongDetails() {
		restTemplate.put("/posts/7676", post);
		ResponseEntity<List> post = restTemplate.getForEntity("/posts/786", List.class);
		assertThat(post.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
	}

	@Test
	public void testFordeletePost() {
		restTemplate.delete("/posts/103", Post.class);
		ResponseEntity<List> post = restTemplate.getForEntity("/posts/103", List.class);
		assertThat(post.getStatusCode()).isEqualTo(HttpStatus.OK);

	}
	@Test
	public void testFordeletePostWithIdNotPresent() {
		restTemplate.delete("/posts/104", Post.class);
		ResponseEntity<List> post = restTemplate.getForEntity("/posts/104", List.class);
		assertThat(post.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
